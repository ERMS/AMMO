...
<div style=" position:relative; width:700px; display:block; margin: 0 auto;">
	...
	<div class="container" style="margin-top:125px;">
		...
		<?php
	$query = mysqli_query($con, "SELECT * FROM person WHERE ID_Number LIKE 'S%'");
	if (mysqli_num_rows($query)<=0)
		echo "No Added Users";
	else {
		?>
		...
		while ($row = mysqli_fetch_array($query)){
			...
			echo "<td><form method='GET' action='adduser.php'> <button name='delete' value='".$row['ID_Number']."''> Delete </button> </form></td></tr>";
		}
		?>
		</table>
